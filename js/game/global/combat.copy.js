...
class _combats {
    ...
    intitialize_monstersPositions(dataObj = this.initialCase){
       ...
       // avaible.remove($player2.inCase); TODO:
        ...
        for (let i=0, l=avaible.length; i<l; i++) {
            ...
            const dist = $camera.getDistanceFrom( $players.p0.p, cases.p );
            if( dist.d > idealDistance ){
                const monster = monsters.pop();
                monster.p.position3d.copy(cases.p.position3d);
    ...
    intitialize_background(){
        ...
        Background.d.alpha = 0;
        Background.n.alpha = 0;
    };
    ...
    intitialize_huds(){
        $gui.Minimap.hide();
        $gui.CombatSelector.initialize(this.battlers);
    ...
    battleStartAnimations(){
        ...
        const Background = $stage.scene.Background;
        const BackgroundCombat = Background.c;
        ...
        const initMask = () => {
            const tl = new TimelineMax();
                tl.call(() => { // the scale of mask is update in 'doShockWave' ticks and sync with ShockwaveFilter.time
                    ...
                    BackgroundCombat.mask = maskCircleSprite;
        ...
        const initShockWave = () => {
            const tl = new TimelineMax();
                tl.call(() => {
                    ...
                    BackgroundCombat.filters = [ShockwaveFilter];
        ...
        const doShockWave = () => {
            const tl = new TimelineMax();
                tl.fromTo(BackgroundCombat, 1, {alpha:0.2},{alpha:1, ease:Power3.easeOut },0);
        ...
        const ending = () => {
            ...
            tl.call(() => {
                ...
                BackgroundCombat.mask = null;
                BackgroundCombat.filters = null;
