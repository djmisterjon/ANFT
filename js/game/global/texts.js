...
class _Texts{
    ...
    */
    ...
    /** return text matrice */
    txtMatrice(txt,styleID,type,origine=0){
        return {txt,styleID,type};
    };
    computeMessages(messageId,originalTxt,target, defaultStyle=9, lineStyle = this.bubbleLineStyles){
        let txt = originalTxt;
        let matrix = [];
        let currentStyleId = defaultStyle;
        let re = /\<(#|S|P|N|I)(\w+|)\>/;
        let match;
        for (let match; match = re.exec(txt);) {
            ...
            switch (match[1]) { // type de tags trouver?
                case 'S': //#STYLE
                    currentStyleId = match[2]?+match[2]:0;
                break;
        ...
        const pagesMatrix = this.matrixMetric(matrix,lineStyle);
        return this.matrixSprites(messageId,target,pagesMatrix);
    }

    /** calcule  et mesure les strings, ajouter les data metric */
    matrixMetric(matrix,lineStyle){
        /** contien les id matrix pour les pages */
        const pages = [[]];
        let currentWidth = 0;
        for (let i=0,p=0,line=0; i<matrix.length; i++) {
            const tm = matrix[i];
            let TextMetrics = PIXI.TextMetrics.measureText(tm.txt, $systems.styles[tm.styleID] )
            if(tm.type === "P"){ // nouvelle page?
                pages[p].push(tm);
                pages.push([]);
                p++;
                line = 0;
            ...
            if(tm.type === "N"){ // nouvelle page?
                pages[p].push(tm);
                currentWidth = 0;
                line++;
                continue;
            };
            //! si depasse height page
            if(line>=4){
                matrix.splice(i, 0, this.txtMatrice( '', tm.styleID, "P" ));//ajout un saut page
                i--;
                continue;
            }
            //! si la ligne depass le maxWidth
            if(currentWidth+TextMetrics.width>lineStyle.maxWidth){
                const letterWidth = TextMetrics.width/TextMetrics.text.length;
                const overWidth = currentWidth+TextMetrics.width-lineStyle.maxWidth;
                const splitedWords = tm.txt.match(/\S+./g);
                const index = this.findIndexSplitedWord(splitedWords,letterWidth,overWidth) //~~(overWidth/letterWidth);
                tm.txt = splitedWords.slice(0,index).sum(); // replace le txt qui peut fitter
                matrix.splice(i+1, 0, 
                    this.txtMatrice( '', tm.styleID, "N",1 ),//ajout un saut ligne
                    this.txtMatrice( splitedWords.slice(index).sum(), tm.styleID, tm.type )//ajoute lexedant dans une nouvelle matrice
                );
                TextMetrics = PIXI.TextMetrics.measureText(tm.txt, $systems.styles[tm.styleID] ); // replace
            };
    ...
    findIndexSplitedWord(splitedWords,letterWidth,overWidth){
        for (let i=splitedWords.length-1; ; i--) {
            const word = splitedWords[i];
            if(overWidth-(word.length*letterWidth)<=0){
                return i;
            }
            overWidth-= word.length*letterWidth;
        };
        return -1;
    };
    
    /** creer les sprite txt */
    matrixSprites(messageId,target,pagesMatrix,splitby = 'letter'){
        const messages = [];
        pagesMatrix.forEach(matrix => {
            ...
            for (let i=0, l=matrix.length; i<l; i++) {
                ...
                if(mt.type === "N"){
                    x=0;
                    y+=40; // todo dans matrix data? maxLineheight
                    continue;
                }
                if(mt.type === "I"){
                    
                }else{
                    if(splitby === 'letter'){ // motionText by letter
                        mt.txt.split('').forEach(letter => {
                            const txt = new PIXI.Text(letter, $systems.styles[mt.styleID]);
                    ...
                    if(splitby === 'word'){
                        ...
                        const txt = new PIXI.Text(mt.txt, $systems.styles[mt.styleID]);
                        txt.position.set(x,y);
                        container.addChild(txt);
                        x+=txt.width;
                    }
            ...
            if('allow options motions sprites'){
                motionsTexture = new PIXI.Sprite( $app.renderer.generateTexture(container) );
            }
            messages.push(this.addDataMessages(messageId, originTxt, target, container,motionsTexture));
        });
        return messages;
    };
